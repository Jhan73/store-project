...
|----|--------------|-------|------|-----------|--------|
| M1-B1 | **Shared foundations**: `Money`, `Ids`, `Clock`, base entity, `CurrentActor`, `ErrorCode`/`BusinessException` + global advice, `PageResponse`, correlation ID, `MeterFilter` allowlist, ArchUnit rules (`@PreAuthorize`, no `now()`) | — | §4.11–4.13, §5.1, §5.3, §7.1, §12 | Error bodies match §5.1 for MVC, validation, and unexpected errors; ArchUnit rules fail on violations | doing |
