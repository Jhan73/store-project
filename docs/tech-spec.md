...
- Refresh tokens are opaque, high-entropy random values (`SecureRandom`, 32 bytes, base64url) returned to the client only once, in the cookie described in §6.4. `identity.refresh_token` stores only their SHA-256 hash (`token_hash`), never the raw value, plus `family_id`, `user_id`, `issued_at`, `expires_at`, `used_at`, `revoked_at`. Default TTL is 30 days (`jugueria.identity.refresh-token.ttl`).
- **Rotation.** Every successful `/auth/refresh` marks the presented token `used_at` and inserts a new row with the same `family_id`, so a family is the chain of tokens issued for one login session. The response carries a new access token in the body and the new refresh token in a fresh cookie, exactly like login.
- **Reuse detection.** A token already marked `used_at`, or already `revoked_at`, or past `expires_at` is rejected; if it was already used (not merely raced, see below), the whole family is revoked with one `UPDATE … WHERE family_id = :id AND revoked_at IS NULL` — a stolen-and-replayed token invalidates every token descended from it, forcing a fresh login.
- **Concurrency.** The rotation itself is one atomic conditional `UPDATE identity.refresh_token SET used_at = :now WHERE id = :id AND used_at IS NULL AND revoked_at IS NULL AND expires_at > :now`, mirroring the lockout counter's pattern (§7.1 above): the affected row count, not a prior `SELECT`, is what decides success. Two concurrent refreshes with the same token race this `UPDATE`; the loser (0 rows) revokes the family exactly like a detected reuse. This means a genuine race also burns the family, including the winner's brand-new token — deliberately: the backend cannot tell a race from an attacker replaying a token milliseconds after the legitimate client, so it treats both as compromise and requires a fresh login. `RefreshTokenService.rotate` runs `@Transactional(noRollbackFor = BusinessException.class)` so the family revocation survives the rejection it causes, the same technique as the lockout counter.
- **Inactive or locked accounts** are refused at the same `UPDATE api/v1/auth/refresh` call (checked before the atomic rotation, so a legitimate token is not burned by an incidental check): combined with the 15-minute access token TTL, this is what makes FR-ADM-01 (deactivation takes effect within 15 minutes) hold — once the access token expires, the client's next refresh finds the account inactive and is refused.
- **`auth.invalid-refresh-token` (401)** covers every rejection above (unknown, expired, revoked, reused, or refused for account state) — deliberately undifferentiated, for the same reason `auth.invalid-credentials` covers every login rejection (§7.1 above).
