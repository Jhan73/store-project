...
| `notifications` | Email (port with two adapters: SMTP to Mailpit locally, SES API v2 with the task role in `test`/`prod` — no stored credentials; a minimal `NotificationsApi.sendStaffSetPasswordEmail` landed at M1-B2, ahead of the full port at M2-B6, with a logging-only local adapter and no working `test`/`prod` transport yet — transport pending (dependencies approved, next slice)), real-time push (STOMP), `LISTEN/NOTIFY` bridge | — | FR-ONL-06, FR-REG-06, NFR-04 |
...
| Money | `@Embeddable Money` (`numeric(12,2)` + `char(3)` currency), columns named `<attribute>_amount` / `<attribute>_currency` through Hibernate's component-path implicit naming. Compare with `compareTo`, never `BigDecimal.equals` (it is scale-sensitive). Every operation that can produce more than two decimals states its rounding explicitly |
...
- **Multi-tab race.** Single-flight only dedupes concurrent refreshes *within one tab* — each browser tab is a separate JS context with its own in-memory access token and its own interceptor instance, so two tabs can each decide to refresh around the same moment against the one shared cookie. The backend's reuse detection (§7.1) cannot tell that race from a stolen-token replay, so it revokes the whole family either way, logging every open tab out. The interceptor **must** serialize refreshes across tabs with the [Web Locks API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Locks_API) (`navigator.locks.request('auth-refresh', async () => { ... })`): a losing tab blocks until the lock is released, then re-reads the in-memory access token (now set by the tab that ran the actual refresh) instead of calling `/auth/refresh` itself. A backend-side grace window (accepting a just-rotated token for a few seconds) was considered and rejected: `identity.refresh_token` stores only hashes, so honoring a reused hash would mean minting a second child token from the same parent and accepting that two children can coexist — which is exactly the weakened reuse detection the whole design exists to avoid, for a race the frontend can prevent for free.
...
- Request `{ email, password }`; response `{ accessToken, tokenType, userId, role }`. Login also issues a refresh token cookie (below).
...
- The failed-attempt counter (`identity.user_account.failed_attempts`/`locked_until`) is updated with a conditional atomic `UPDATE` (native SQL — a JPQL `CASE ... ELSE null` left PostgreSQL unable to infer `locked_until`'s type), not `@Version` optimistic locking: two concurrent failed attempts on the same account must both be counted, and optimistic locking would instead reject the second one as a `409 common.concurrent-modification`, which is the wrong answer for a login attempt. The update — and the `BusinessException` that follows it — happen in the same transaction, marked `noRollbackFor` that exception so the recorded failure survives the login being rejected. `@Transactional` stays only on `LoginService`, never on the repository, per §4.11; a repository test that needs a standalone transaction uses `TransactionTemplate`.
- **Lock expiry restarts the count, it does not resume it.** If `locked_until` has already passed when a new failure is recorded, the atomic update sets `failed_attempts` back to 1 instead of incrementing the stale value — otherwise a single failed attempt right after expiry would immediately relock the account for another full lockout window, and the lockout would never actually end under light, continuous traffic.
- **Unknown email, wrong password, and an inactive account cost the same BCrypt work.** `LoginService` always calls `passwordEncoder.matches(...)`, even when there is no account to check against — in that case against a dummy hash computed once at construction with the same encoder. Skipping the check for an unknown or inactive account would make those branches measurably faster than a wrong password on a real, active account, leaking through response timing exactly the distinction `auth.invalid-credentials` is meant to hide.
...
- `created_at`/`updated_at` on `user_account` are set from the injected `Clock`, in the constructor and in the same atomic `UPDATE`s that touch the row — never a Hibernate `@CreationTimestamp`/`@UpdateTimestamp` generator, which stamps from its own internal clock and would drift out of step with a `Clock` the tests can fix or advance. An ArchUnit rule (`NO_HIBERNATE_GENERATED_TIMESTAMPS`) forbids both annotations anywhere in `src/main`.

**Refresh token rotation** (`POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`, M1-B2):

- Refresh tokens are opaque, high-entropy random values (`SecureRandom`, 32 bytes, base64url) returned to the client only once, in the cookie described in §6.4. `identity.refresh_token` stores only their SHA-256 hash (`token_hash`), never the raw value, plus `family_id`, `user_id`, `issued_at`, `expires_at`, `family_started_at`, `used_at`, `revoked_at`.
- **Rotation.** Every successful `/auth/refresh` marks the presented token `used_at` and inserts a new row with the same `family_id` and the same `family_started_at`, so a family is the chain of tokens issued for one login session and knows exactly when that session began. The response carries a new access token in the body and the new refresh token in a fresh cookie, exactly like login.
- **Idle TTL vs. absolute lifetime.** Two independent limits, both owner decisions:
  - *Idle TTL* (`jugueria.identity.refresh-token.idle-ttl`, default 7 days): how long an unused token stays valid — resets on every rotation, like a sliding session.
  - *Absolute lifetime* (`jugueria.identity.refresh-token.absolute-ttl`, default 30 days): a hard ceiling from the family's `family_started_at` (the original login), independent of how often the client refreshes. A session that refreshes daily for 40 days still ends at day 30.
  - Each issued token's `expires_at` = `min(now + idle-ttl, family_started_at + absolute-ttl)`. Login starts a new family (`family_started_at = now`), so its first token is bounded only by the idle TTL until the family gets old enough for the absolute ceiling to bind.
  - The cookie's `Max-Age` is the seconds remaining until that specific token's `expires_at` — **not** a fixed TTL — so it visibly shrinks once the absolute ceiling starts capping it.
  - The absolute ceiling is enforced twice: once as an early rejection (`RefreshToken.exceedsAbsoluteLifetime`, checked before touching the account or rotating, so a stale family never burns a token unnecessarily) and again inside the atomic rotation `UPDATE` itself (below), because a family can cross the ceiling between the early check and the update under a real race.
- **Reuse detection.** A token already marked `used_at`, or already `revoked_at`, or past its own `expires_at`, or whose family is past the absolute ceiling, is rejected; if it was already used (not merely raced, see below), the whole family is revoked with one `UPDATE … WHERE family_id = :id AND revoked_at IS NULL` — a stolen-and-replayed token invalidates every token descended from it, forcing a fresh login.
- **Concurrency.** The rotation itself is one atomic conditional `UPDATE identity.refresh_token SET used_at = :now WHERE id = :id AND used_at IS NULL AND revoked_at IS NULL AND expires_at > :now AND family_started_at > :absoluteFloor` (`absoluteFloor = now - absolute-ttl`, so the comparison needs no SQL interval arithmetic), mirroring the lockout counter's pattern (§7.1 above): the affected row count, not a prior `SELECT`, is what decides success. Two concurrent refreshes with the same token race this `UPDATE`; the loser (0 rows) revokes the family exactly like a detected reuse. This means a genuine race also burns the family, including the winner's brand-new token — deliberately: the backend cannot tell a race from an attacker replaying a token milliseconds after the legitimate client, so it treats both as compromise and requires a fresh login (see §6.4 for why the frontend must prevent this race itself). `RefreshTokenService.rotate` runs `@Transactional(noRollbackFor = BusinessException.class)` so the family revocation survives the rejection it causes, the same technique as the lockout counter.
- **Inactive or locked accounts** are refused before the atomic rotation, so a legitimate token is not burned by an incidental check: combined with the 15-minute access token TTL, this is what makes FR-ADM-01 (deactivation takes effect within 15 minutes) hold — once the access token expires, the client's next refresh finds the account inactive and is refused.
- **`auth.invalid-refresh-token` (401)** covers every rejection above (unknown, idle-expired, past the absolute lifetime, revoked, reused, or refused for account state) — deliberately undifferentiated, for the same reason `auth.invalid-credentials` covers every login rejection (§7.1 above).
- **CSRF defense.** Neither endpoint carries a bearer token, so both require the `X-Requested-With` header (§6.4); Spring's standard `@RequestHeader` binding rejects a missing header as `400 common.malformed-request` (§5.1) without any custom filter.
- **Cookie.** `__Host-refresh-token`; `HttpOnly`, `Secure`, `SameSite=Strict`, `Path=/`, `Max-Age` per token as described above, or `0` (logout, clearing it). Built with `ResponseCookie`, never a hand-written header string. Locally, the app runs over plain HTTP (§8.5): a `Secure` cookie is not stored by a real browser there, a known gap tracked against serving local dev over HTTPS; it does not affect `test`/`prod` (both HTTPS behind CloudFront) or the backend test suite, which asserts the cookie's attributes directly through `MockMvcTester` rather than a real browser.
- **Logout** revokes the presented token's whole family and always answers `204` with the cookie cleared, whether or not the cookie was present or the token still valid — no information is revealed about why a token failed, and repeated calls are safe.
- **Revocation hook.** `RefreshTokenService.revokeAllForUser(userId)` revokes every refresh token of a user regardless of family, for a later slice (M1-B2 continuation) to call on deactivation or role change.

**Staff administration** (`/api/v1/staff/**`, ADMIN only, M1-B2):

- `POST /api/v1/staff` (create), `GET /api/v1/staff` (paginated list), `GET /api/v1/staff/{id}`, `PATCH /api/v1/staff/{id}/role`, `POST /api/v1/staff/{id}/deactivate`, `POST /api/v1/staff/{id}/reactivate`, `POST /api/v1/staff/{id}/set-password-link` (resend, below). All `@PreAuthorize("hasRole('ADMIN')")`. List and get exclude `CUSTOMER` rows (`404`, not `403` — ownership-style hiding, §7.1 above).
- **No admin-chosen or temporary password.** `POST /api/v1/staff` starts the account with an unusable password hash (a random value never handed to anyone) and issues a single-use set-password token, mirroring the refresh-token design: opaque `SecureRandom` value (`RefreshTokens.newRawToken()`/`.hash()`, reused as-is), only its SHA-256 hash stored in `identity.set_password_token`, default expiry `jugueria.identity.set-password.token-ttl` (48h). A new token invalidates every earlier unused token of the same user (`revoked_at`).
- **Resend** — `POST /api/v1/staff/{id}/set-password-link`: a token expires in 48h with no way back in otherwise, so an admin can reissue one for a staff member who missed the window or was reactivated without ever setting a password. Not for one's own account (same `identity.cannot-modify-own-account` guard as deactivate/role-change). Reissuing revokes the earlier unused token, builds a new link, and sends it through the same `NotificationsApi` call described below. `204` on success.
- `POST /api/v1/auth/set-password` (`@PermitAll`, in `PUBLIC_ROUTES`, requires `X-Requested-With` like refresh/logout): `{ token, newPassword }` → `204`. Account state is checked **before** the token is consumed, mirroring refresh-token rotation's own ordering (§7.1 above): an inactive account is rejected without burning the token, so a re-activation lets a still-valid link work again. Consumption itself uses the same conditional-`UPDATE`-plus-row-count pattern as refresh-token rotation (`0` rows ⇒ `auth.invalid-set-password-token`, undifferentiated for unknown/expired/used/revoked/inactive-account, same rationale as `auth.invalid-credentials`). Resets `failed_attempts`/`locked_until`. Does **not** log the caller in (owner decision) — the frontend redirects to login after `204`.
- **Password policy** (tech-spec gap filled here, owner decision): minimum length 12, maximum 100, no composition rules — NIST 800-63B favors length over forced complexity. Revisit if a stronger policy is required later.
- **Guards**, both new `identity.*` codes: an ADMIN cannot deactivate, change the role of, or resend a link for their own account (`identity.cannot-modify-own-account`, `422`) and the last active `ADMIN` cannot be deactivated or demoted (`identity.last-active-admin-required`, `409` — another admin being promoted can change this). Duplicate email is `identity.email-already-registered` (`409`, the canonical example already named in `backend/CLAUDE.md`). Assigning `CUSTOMER` through this API is `identity.invalid-staff-role` (`422`).
- **Serializing the last-active-admin invariant.** Reading "how many active ADMINs remain" and then deciding to demote or deactivate one is a check-then-act: two concurrent requests against the last two active ADMINs can each read 2, each pass the check, and leave 0. `changeRole`, `deactivate`, and first-admin creation each start their transaction with `pg_advisory_xact_lock` on one fixed key (`ActiveAdminLock`, held until commit or rollback), so a second concurrent caller blocks until the first's count-and-mutate finishes rather than racing it. Chosen over `SELECT … FOR UPDATE` on the active-admin rows because there is no single row set to lock ahead of time — the count itself is the invariant, not a set of pre-identified rows.
- Deactivation and role change call `RefreshTokenService.revokeAllForUser` in the same transaction (closing the 15-minute FR-ADM-01 window per the "Revocation hook" above) and, for deactivation only, also revoke any unused set-password token — a deactivated account's outstanding invite link stops working too. Reactivation does not revoke or re-issue anything.
- **Events.** Only the three the module already declares (§4.1): `UserCreated`, `UserDeactivated`, `UserRoleChanged`, published via `ApplicationEventPublisher` (no consumer yet — `audit`, M1-B3, is the intended one; publishing now avoids a gap once it lands). Role change publishes only when the role actually differs; reactivation and resend publish nothing, since the tech-spec defines no event for either — a known gap, tracked for whoever picks up `audit`.
- **Email delivery.** `notifications.NotificationsApi.sendStaffSetPasswordEmail(toEmail, link, expiresAt)` is the minimal public surface added ahead of M2-B6's full port. `identity` builds the link itself (`jugueria.identity.set-password.frontend-base-url` + `.frontend-path`, default `/set-password` — the frontend page does not exist yet) and calls it **after** the creating transaction commits, never inside it (external systems are never called inside a transaction, §4.11). A failed send does not fail account creation; it is logged at `ERROR` without the link or token and the account still exists with its token, recoverable manually (or via resend, above). The email subject/body live in `notifications/staff-set-password-email.txt` (no text in Java, §6.6); the full template *engine* decision stays open for M2-B6.
  - **Transport pending.** The owner has approved `spring-boot-starter-mail` (SMTP, local/Mailpit) and `software.amazon.awssdk:sesv2` (`test`/`prod`) for D16, but neither is added in this slice — that lands as a separate slice. `notifications.internal.LoggingEmailSender` (`@Profile("local")`) is the only adapter allowed to expose a link today, and only via the app logger at `INFO` in that one profile. `notifications.internal.UnavailableEmailSender` (`@Profile("!local")`) fails loudly everywhere else, including the current test suite (no profile active), without logging anything. **`test`/`prod` cannot deliver this email until that dependency slice lands.**
- **First-admin bootstrap** (`docs/runbooks/first-admin-bootstrap.md`): the backend binary run once as `--bootstrap-first-admin --admin-email=<email>` (an ECS one-off task in `test`/`prod`, `WebApplicationType.NONE` so it never binds a port). Idempotent — a no-op once an active `ADMIN` exists, the check and the creation happening under the same `ActiveAdminLock` as above so two concurrent bootstrap runs cannot both create a "first" admin. Reuses the same staff-creation path, then prints the created email and the set-password link to the process's own `stdout` only (never the application logger, in any profile) and exits (`SpringApplication.exit`). This is the one deliberate exception to "never log the link": the alternative, no working email transport, would otherwise make bootstrapping an environment impossible before the transport dependency lands.
