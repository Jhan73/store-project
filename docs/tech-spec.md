...
- `POST /api/v1/staff` (create), `GET /api/v1/staff` (paginated list), `GET /api/v1/staff/{id}`, `PATCH /api/v1/staff/{id}/role`, `POST /api/v1/staff/{id}/deactivate`, `POST /api/v1/staff/{id}/reactivate`. All `@PreAuthorize("hasRole('ADMIN')")`. List and get exclude `CUSTOMER` rows (`404`, not `403` — ownership-style hiding, §7.1 above).
...
- `POST /api/v1/auth/set-password` (`@PermitAll`, in `PUBLIC_ROUTES`, requires `X-Requested-With` like refresh/logout): `{ token, newPassword }` → `204`. Consumes the token with the same conditional-`UPDATE`-plus-row-count pattern as refresh-token rotation (`0` rows ⇒ `auth.invalid-set-password-token`, undifferentiated for unknown/expired/used/revoked, same rationale as `auth.invalid-credentials`). Resets `failed_attempts`/`locked_until`. Does **not** log the caller in (owner decision) — the frontend redirects to login after `204`.
...
- **Guards**, both new `identity.*` codes: an ADMIN cannot deactivate or change the role of their own account (`identity.cannot-modify-own-account`, `422`) and the last active `ADMIN` cannot be deactivated or demoted (`identity.last-active-admin-required`, `409` — another admin being promoted can change this). Duplicate email is `identity.email-already-registered` (`409`, the canonical example already named in `backend/CLAUDE.md`). Assigning `CUSTOMER` through this API is `identity.invalid-staff-role` (`422`).
