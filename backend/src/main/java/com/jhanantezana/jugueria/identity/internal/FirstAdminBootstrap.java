package com.jhanantezana.jugueria.identity.internal;

import org.jspecify.annotations.Nullable;
import org.springframework.stereotype.Component;

import com.jhanantezana.jugueria.shared.Role;

// Idempotent: safe to rerun. Only ever prints to stdout of this one-off run, never through the app logger.
@Component
class FirstAdminBootstrap {

	private final UserAccountRepository accounts;

	private final StaffProvisioningService provisioning;

	FirstAdminBootstrap(UserAccountRepository accounts, StaffProvisioningService provisioning) {
		this.accounts = accounts;
		this.provisioning = provisioning;
	}

	int run(@Nullable String adminEmail) {
		if (accounts.countByRoleAndActiveTrue(Role.ADMIN) > 0) {
			System.out.println("An active ADMIN already exists; nothing to do.");
			return 0;
		}
		if (adminEmail == null || adminEmail.isBlank()) {
			System.err.println("Missing required --admin-email=<email> argument.");
			return 1;
		}
		var provisioned = provisioning.createStaff(adminEmail, Role.ADMIN);
		System.out.println("First ADMIN created: " + provisioned.account().getEmail());
		System.out
			.println("Set-password link (expires " + provisioned.linkExpiresAt() + "): " + provisioned.setPasswordLink());
		return 0;
	}

}
